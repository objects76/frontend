...
function ns_palette() {
    ...
    let ctx = canvas ? canvas.getContext('2d') as CanvasRenderingContext2D : null;
    ...
    */
    ...
    function on_palette_type(obj: HTMLSelectElement) {
        let palette = null;
        switch (obj.value) {
        ...
        drawPalette(10, 10, palette);
    }


    // https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D
    function drawPalette(initx:number, y:number, colors:any) {
        if (!ctx) return;
        ...
        let x = initx;
        let i = 0;
        for (let c of colors.clrs as PaletteEntry[]) {
    ...
    function resize() {
        ...
        on_palette_type(document.querySelector('#palette_type') as HTMLSelectElement);
    }
    window.addEventListener('load', resize);
    window.addEventListener("resize", resize, false);

    let ui_palette_type = document.querySelector('#palette_type') as HTMLSelectElement;
    ui_palette_type.addEventListener('change', ()=>{on_palette_type(ui_palette_type);});
};
ns_palette();